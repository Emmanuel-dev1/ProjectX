// components/Header.tsx (with original logo)
  ...
  return (
    <header className="sticky top-0 z-50 bg-dark-gradient shadow-header">
      <div className="max-w-8xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between relative">
        {/* Logo - Always visible - RESTORED TO ORIGINAL */}
        ...
        <div className="flex items-center gap-4">
          ...
          {isAuthenticated ? (
            <div ref={userMenuRef} className="relative">
              ...
              >
                ...
                <div className="hidden sm:block text-left">
                  <div className="text-sm font-medium truncate max-w-[120px]">{userDisplay.name}</div>
                  <div className="text-xs text-gray-400">{userDisplay.role}</div>
                </div>
                ...
                <div className="absolute top-full right-0 mt-2 w-64 sm:w-72 bg-white rounded-xl shadow-lg border border-border-light z-50 overflow-hidden animate-slide-down max-h-[80vh]">
                  ...
                  <div className="px-4 sm:px-5 py-4 border-b border-border-light bg-bg-light">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-semibold text-text-primary truncate">{userDisplay.name}</div>
                        <div className="text-xs text-text-secondary truncate">{user?.email || 'user@example.com'}</div>
                      </div>
                    </div>
                    <Link
                      to="/profile"
                  ...
                  <div className="py-2 max-h-[50vh] overflow-y-auto custom-scrollbar">
                    ...
                    <button
                      onClick={toggleTheme}
                      className="w-full flex items-center justify-between gap-3 px-4 sm:px-5 py-3 cursor-pointer transition-colors text-sm text-left text-text-primary hover:bg-bg-light dark:hover:bg-bg-dark"
                    >
                      <div className="flex items-center gap-3">
                        {theme === 'dark' ? (
                          <Sun size={16} className="text-text-secondary opacity-70" />
                        ) : (
            ...
            <div className="flex items-center gap-2">
              <Link
                to="/login"
                className="flex items-center gap-2 text-gray-300 hover:text-white px-3 py-2 rounded-md hover:bg-white/10 transition-colors"
              >
                <LogIn size={18} />
              ...
              <Link
                to="/register"
                className="bg-accent-green text-text-primary px-4 py-2 rounded-md font-medium hover:bg-green-300 transition-colors flex items-center gap-2"
              >
                <UserPlus size={18} />
              ...
              {isAuthenticated ? (
                <>
                  <div className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-white truncate">{userDisplay.name}</div>
                      <div className="text-xs text-gray-400 truncate">{userDisplay.role}</div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 px-4">
                  ...
                  <Link
                    to="/register"
                    onClick={() => setMobileMenuOpen(false)}
                    className="text-center bg-accent-green text-text-primary rounded-md py-2 text-xs font-medium hover:bg-green-300 transition-colors"
                  >
                    Sign Up
                  </Link>
                </div>
